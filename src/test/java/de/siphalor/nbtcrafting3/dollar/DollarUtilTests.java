...
public class DollarUtilTests {
	...
	void asBoolean_truthy() {
		...
		NbtList someList = new NbtList();
		someList.add(NbtInt.of(1));
		Assertions.assertTrue(DollarUtil.asBoolean(someList));
		NbtCompound someCompound = new NbtCompound();
		someCompound.put("someKey", NbtInt.of(1));
	...
	void asBoolean_falsy() {
		...
		Assertions.assertFalse(DollarUtil.asBoolean(new NbtList()));
		Assertions.assertFalse(DollarUtil.asBoolean(new NbtCompound()));
	}
	...
	void expectNumber() {
		...
		Assertions.assertThrows(DollarException.class, () -> DollarUtil.expectNumber(new NbtList()));
		Assertions.assertThrows(DollarException.class, () -> DollarUtil.expectNumber(new NbtCompound()));
	}
}
